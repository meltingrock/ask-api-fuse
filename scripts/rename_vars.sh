...
#
# USAGE:
#   ./rename_fuse_to_fuse.sh <TARGET_DIRECTORY>
#
# EXAMPLE:
#   ./rename_fuse_to_fuse.sh /path/to/your/repo
#
# REQUIRES:
#   - bash
#   - find
...
This script will recursively rename "FUSE" -> "FUSE" and "fuse" -> "fuse" 
...
echo "=== Step 1: Replacing 'R2R' -> 'FUSE' and 'fuse' -> 'fuse' in file contents ==="
if [[ "$(uname -s)" == "Darwin" ]]; then
  # macOS (BSD sed requires a backup extension with -i)
  find "$TARGET_DIR" -type f -not -path '*/.git/*' -not -path '*/scripts/*' \
    -not -path '*/__pycache__/*' \
    -exec sed -i '' 's/R2R/FUSE/g; s/r2r/fuse/g; s/meltingrock/meltingrock/g' {} +
else
